...
sub new
{
    ...
    my ($type) = $cfgHash->{"pundit_agent"}{$site}{"reporting"}{"type"};
    
    my $reporter;
    if ($type eq "mysql") 
    {
        $reporter = new Detection::Reporter::MySQL($cfgHash, $site);
    }
...
sub writeStatus
{
    ...
    if ($@)
    {
        ...
        print "Failed to write status to server. Discarding status from " . $status->{'srchost'} . " to " . $status->{'dsthost'} . " at " . $status->{'startTime'} . "\n";
    }
}

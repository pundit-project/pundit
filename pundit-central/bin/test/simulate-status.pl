...
while (1) {
  ...
  $message = "test.siteA.org|test.siteB.org|8.66127014160156|";

  while ($lastTime < $targetTime) {
    my $startTime = $lastTime - 5;
    my $endTime = $lastTime;
    my $detectionCode = 0;
    my $queueingDelay = (int(rand(100)) + 800)/100;
    my $lossRatio = 0;
    my $reorderMetric = 0;
    my $statusText = "$startTime,$endTime,$detectionCode,$queueingDelay,$lossRatio,$reorderMetric;";
